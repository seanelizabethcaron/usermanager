...
for username in report:
    ...
    query = 'UPDATE trainings SET dce101_comp = ' + str(dce101_completed) + ' WHERE serialnum = ' + str(username[0]) + ';'
    local_curs.execute(query)

    query = 'UPDATE trainings SET bulkdata_comp = ' + str(bulkdata_completed) + ' WHERE serialnum = ' + str(username[0]) + ';'
    local_curs.execute(query)

    query = 'UPDATE trainings SET itse106_comp = ' + str(itse106_completed) + ' WHERE serialnum = ' + str(username[0]) + ';'
    local_curs.execute(query)
