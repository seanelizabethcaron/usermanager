...
def generate_html_header():
    ...
    print('<b>User Account Manager Requester Dashboard</b>')
    print('</div>')
    print('</div>')
...
try:
    requester = os.environ["REMOTE_USER"]
...
if fm_selectuser:
    ...
    if debug:
        ...
        debuglog.write(debug_time + ': Requester dashboard got form data:\n')
    ...
    curs = db.cursor()
    query = 'SELECT * from users where requestor = \'' + requester + '\' and created = 1;'
    curs.execute(query)
    users = curs.fetchall()
